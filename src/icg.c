/*
 * icg.c  —  StudyLang Intermediate Code Generator
 *
 * Walks the AST one statement at a time and emits:
 *   (a) Three-Address Code (TAC) — the classic compiler IR
 *   (b) A human-readable study plan table at the end
 *
 * TAC instruction set used:
 *   LOAD_ACTION   <verb>           -> tN
 *   LOAD_SUBJECT  <subject>        -> tN
 *   LOAD_DUR      <N units>        -> tN
 *   SET_DUR       tX, tN           -> tN
 *   LOAD_TREF     <prep ref>       -> tN
 *   SET_TREF      tX, tN           -> tN
 *   LOAD_DAY      <prep day>       -> tN
 *   SET_DAY       tX, tN           -> tN
 *   SET_PRIORITY  <level>          -> tN
 *   BIND_PRIORITY tX, tN           -> tN
 *   SCHEDULE      tX, tY           -> tN   (finalise the task)
 *   SEQ_POINT                              (separator between tasks)
 */
...
static int      temp_n    = 0;   /* next temporary variable number */
...
/* Convert a string to uppercase in-place into dst */
...
static void print_study_plan(void) {
    ...
    for (int i = 0; i < task_count; i++) {
        ...
        /* Combine modifier + action into one cell */
        char action_full[80] = "";
        if (t->modifier[0])
        ...
        /* Combine timeref + day into one cell */
...
/* ---- Code generation for one STATEMENT node --------------- */
static void gen_statement(ASTNode *stmt) {
    ...
    /* Classify children by node type */
    for (int i = 0; i < stmt->child_count; i++) {
        ...
        switch (c->type) {
            ...
            case NT_TIME_REF:
                /* day_ref and time_ref are both NT_TIME_REF;
                   tell them apart by the label set in the parser */
    ...
    /* ---- Fill in study plan summary row ------------------- */
    ...
    /* PRIORITY -> SET_PRIORITY + BIND_PRIORITY */
    if (priority) {
        new_temp(t3, sizeof(t3));
    ...
    /* Final: SCHEDULE combines action-chain with subject */
    new_temp(tmp, sizeof(tmp));
...
static void gen_node(ASTNode *node, int *stmt_count) {
    ...
    switch (node->type) {
        ...
        case NT_STATEMENT:
            ...
            break;
        case NT_CONNECTOR:
            break;  /* connectors are structural, no code to emit */
        default:
            break;
    }
}
